...
userSchema.virtual("isKYCVerified").get(function () {
  return (
    ...
    (this.kyc.review?.result === "GREEN" ||
      this.kyc.frontendReview?.reviewAnswer === "GREEN") &&
...
=======
    reviewResult:
      this.kyc.frontendReview?.reviewAnswer || this.kyc.review?.result,
    hasToken: !!this.kyc.verificationToken && !this.kyc.verificationToken.used,
    tokenType: this.kyc.verificationToken?.tokenType || "NONE",
    hasGreenToken: this.hasGreenToken, // Include GREEN token status
    metadata: this.kyc.metadata,
...
=======
userSchema.methods.generateGreenVerificationToken = async function (
  additionalData = {}
) {
  const crypto = require("crypto");
  ...
  const securityHash = crypto
    .createHash("sha256")
    .update(`${this._id}${this.email}${timestamp}`)
    .digest("hex")
    .substring(0, 16);
